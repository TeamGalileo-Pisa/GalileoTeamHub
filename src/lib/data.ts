...
export async function getAreaBookingLink(areaId: string): Promise<string> {
  const { data, error } = await supabase.rpc("get_area_booking_link", {
    p_area_id: areaId,
  });
  throwIfError(error);
  return asString(data);
...
export async function revokeAreaBookingLink(areaId: string): Promise<void> {
  const { error } = await supabase.rpc("revoke_area_booking_link", {
    p_area_id: areaId,
  });
  throwIfError(error);
}
