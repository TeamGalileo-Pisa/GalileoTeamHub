...
vi.mock("../lib/data", () => ({
  createPublicBooking: mocks.book,
...
describe("public booking screen", () => {
  it("shows every missing field and does not call booking API", async () => {
    mount(<PublicBookingPage />, "/book/test");
    fireEvent.click(
      await screen.findByRole("button", { name: "Conferma prenotazione" }),
    );
    await screen.findByText("Inserisci il nome.");
    expect(screen.getByText("Inserisci il cognome.")).toBeInTheDocument();
    expect(
      screen.getByText("Inserisci il tuo indirizzo email universitario."),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText("Seleziona prima uno degli orari disponibili."),
    ).toHaveLength(2);
  ...
  it("refuses non-university emails visibly", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Email universitaria"), {
      target: { value: "mario@gmail.com" },
    });
    fireEvent.click(
      screen.getByRole("button", { name: "Conferma prenotazione" }),
    );
    expect(
      await screen.findByText(/Per prenotare il colloquio devi utilizzare/),
    ).toBeInTheDocument();
    expect(mocks.book).not.toHaveBeenCalled();
  });
  it("shows the requested success screen only after server confirmation", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Email universitaria"), {
      target: { value: " Mario@STUDENTI.UNIPI.IT " },
    });
    fireEvent.click(
      screen.getByRole("button", { name: "Conferma prenotazione" }),
    );
    expect(
      await screen.findByRole("heading", {
        name: "Grazie per la tua prenotazione!",
      }),
    ).toBeInTheDocument();
...
describe("daily availability and calendar UI", () => {
  it("previews all five dates", () => {
    ...
    fireEvent.change(screen.getByLabelText("Data iniziale"), {
      target: { value: "2026-09-21" },
    });
    fireEvent.change(screen.getByLabelText("Data finale"), {
      target: { value: "2026-09-25" },
    });
    expect(
      screen.getByText("Verranno create 5 disponibilità"),
    ).toBeInTheDocument();
  ...
  it("does not offer cross-area filters to area leads", async () => {
    mocks.admin = false;
    mount(<CalendarPage />);
    await screen.findByText(/Nessun appuntamento/);
