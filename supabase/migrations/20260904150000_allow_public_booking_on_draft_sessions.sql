...
-- Area links are intentionally public even while the underlying interview
-- session is still draft. The availability function already exposes those
-- slots; the booking transaction must enforce the same rule.
