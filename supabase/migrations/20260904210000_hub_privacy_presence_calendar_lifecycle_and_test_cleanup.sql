...
-- GalileoHub production hardening:
-- 1) legal documents + explicit privacy consent;
-- 2) online presence for area leads;
-- 3) calendar includes both bookings and still-free slots;
-- 4) explicit soft-cancel vs irreversible delete operations;
-- 5) one-time removal of all scheduling/recruitment TEST data while preserving
--    every account, role, membership, area, room and stable per-area link.

-- Canonicalize columns introduced by the historical trash migrations so a new
-- environment remains reproducible even though those historical files are now
-- migration-history placeholders.
...
-- ---------------------------------------------------------------------------
-- Legal documents and privacy acceptance
-- ---------------------------------------------------------------------------
...
values
  (
    'privacy',
    'Informativa privacy per la prenotazione dei colloqui',
  ...
  (
    'terms',
    'Termini di servizio GalileoHub',
...
begin
  ...
  set title = pg_catalog.trim(p_title),
      body = pg_catalog.trim(p_body),
      version = version + 1,
      updated_by = auth.uid(),
  ...
  insert into public.audit_logs (
    ...
    auth.uid(), 'staff', 'legal_document.updated', 'legal_document', null,
...
-- Preserve the current booking implementation as a private core. The public
-- five-argument entry point becomes fail-closed so an old Edge Function cannot
-- bypass the mandatory privacy consent after this migration lands.
do $$
begin
...
grant execute on function public.book_public_slot(text,uuid,text,text,text,boolean,bigint) to service_role;

-- ---------------------------------------------------------------------------
-- Online presence (admin can see area leads active in the last 90 seconds)
-- ---------------------------------------------------------------------------
...
set search_path = ''
as $$;
-- replaced below because plpgsql body delimiter is required
$$;

drop function public.list_online_area_leads();
create function public.list_online_area_leads()
returns jsonb
language plpgsql
stable
security definer
    ...
    from (
      select p.id as user_id,
             ...
             coalesce(pg_catalog.jsonb_agg(
               pg_catalog.jsonb_build_object('id', a.id, 'name', a.name::text, 'slug', a.slug::text)
               order by a.name::text
             ) filter (where a.id is not null), '[]'::jsonb) as areas,
             up.last_seen_at,
             up.last_path
      from public.profiles p
      join public.user_presence up on up.user_id = p.id
      join public.area_memberships am
        on am.user_id = p.id and am.role = 'area_lead' and am.ended_at is null
...
-- ---------------------------------------------------------------------------
-- Calendar: bookings + owned free slots in the same time window
-- ---------------------------------------------------------------------------
...
begin
  ...
  delete from public.candidates c where c.id = v_candidate_id
    and not exists (select 1 from public.bookings b where b.candidate_id = c.id);
end;
$$;
...
begin
  ...
  set status = 'cancelled', cancelled_at = pg_catalog.now(), cancelled_by = auth.uid()
  where b.status = 'confirmed'
    and b.slot_id in (select id from public.slots where session_id = p_session_id);

  insert into public.email_deliveries(booking_id, kind, idempotency_key)
  select b.id, 'booking_cancelled', b.id::text || ':booking_cancelled:' || extensions.gen_random_uuid()::text
  from public.bookings b
  where b.status = 'cancelled'
    and b.cancelled_by = auth.uid()
    and b.cancelled_at >= pg_catalog.now() - interval '5 seconds'
    and b.slot_id in (select id from public.slots where session_id = p_session_id);

  update public.slots set status = 'disabled'
  ...
  set status = 'cancelled', cancelled_by = auth.uid(), cancelled_at = pg_catalog.now()
...
begin
  ...
  using public.bookings b, public.slots sl
  where pc.booking_id = b.id and b.slot_id = sl.id and sl.session_id = p_session_id;
  delete from public.email_deliveries d
  using public.bookings b, public.slots sl
  where d.booking_id = b.id and b.slot_id = sl.id and sl.session_id = p_session_id;
  delete from public.audit_logs where entity_id in (
    ...
    union select p_session_id
    union select v_allocation_id
  );
  delete from public.bookings b using public.slots sl
...
begin
  ...
  into v_candidate_ids from public.bookings where slot_id=p_slot_id;
...
begin
  ...
  delete from public.booking_privacy_consents pc using public.bookings b, public.slots sl, public.interview_sessions s, public.area_allocations al
  where pc.booking_id=b.id and b.slot_id=sl.id and sl.session_id=s.id and s.allocation_id=al.id and al.room_availability_id=p_availability_id;
  delete from public.email_deliveries d using public.bookings b, public.slots sl, public.interview_sessions s, public.area_allocations al
  where d.booking_id=b.id and b.slot_id=sl.id and sl.session_id=s.id and s.allocation_id=al.id and al.room_availability_id=p_availability_id;
  delete from public.audit_logs where entity_id in (
...
-- ---------------------------------------------------------------------------
-- One-time production cleanup requested on 2026-09-04.
-- All scheduling/recruitment content currently present is TEST data.
-- Explicitly preserved: auth.users, profiles, system_roles, area_memberships,
-- areas, rooms, area_booking_links, announcements and legal documents.
-- ---------------------------------------------------------------------------
