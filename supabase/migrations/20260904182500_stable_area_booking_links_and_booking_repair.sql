...
-- Canonical public booking URLs: one stable, readable token per Team area.
-- Examples: area-logistica, area-business, area-comunicazione.
-- The token is an identifier, not a secret; authorization remains enforced by
-- the server-side booking functions and RLS/service-role boundaries.

update public.area_booking_links link
set token = 'area-' || area.slug::text
from public.areas area
where area.id = link.area_id
  and link.token is distinct from ('area-' || area.slug::text);
